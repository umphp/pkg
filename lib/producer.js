...
export default function ({ backpack, options, target }) {
  return new Promise((resolve, reject) => {
    ...
    const payloadHeader = makePayloadHeader();

/*
    const afterStripe = [
      () => paddingBuffer(payloadMeter),
      preludeBox,
      paddingBuffer(preludeBox.length)
    ];
*/

    function dropMeterAfter (cb, v) {
      count += 1;
      meter.bytes = 0;
      cb(null, v);
    }
    ...
    multistream((cb) => {
      if (count === 0) {
        return dropMeterAfter(
          cb, fs.createReadStream(target.binaryPath)
        );
      } else
      if (count === 1) {
        return dropMeterAfter(
          cb, bufferStream(paddingBuffer(meter.bytes))
        );
      } else
      if (count === 2) {
        return dropMeterAfter(
          cb, bufferStream(bakeryBox)
        );
      } else
      if (count === 3) {
        return dropMeterAfter(
